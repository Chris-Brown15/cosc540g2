...
'''
The leave all rooms event. Emitted by a client who wants to leave all rooms they are in.
'''
LEAVE_ALL_ROOMS_EVENT = "leaveAllRooms"

'''
The request new room event. Emitted by a client who wants a new room to be created.
'''
...
class SUCChatServer:
	...
	'''
	Called when a client emits a LEAVE_ALL_ROOMS_EVENT. This function receives JSON representing the client's username.
	'''
	def leaveAllRooms(self , jsonString):
		jsonData = json.loads(jsonString)
		for room in self.__userRooms:
			if room.containsUser(jsonData.sender):
				leave_room(room.name)
				room.users.remove(jsonData.sender)


	'''
	Called when a client emits a REQUEST_NEW_ROOM_EVENT. This function receives JSON representing the client's username and the username of the person they want to chat with.
	'''
	def requestNewRoom(self , jsonString):
		jsonData = json.loads(jsonString)
